...
import 'package:qrx_pro/core/services/export/qr_export_service.dart';
...
class _PreviewScreenState extends State<PreviewScreen> {
  ...
  Future<void> _exportQrAsPdf() async {
    ...
    try {
      ...
      if (_logoImage != null) {
        final Uint8List imageBytes = await _logoImage!.readAsBytes();
        final ui.Codec codec = await ui.instantiateImageCodec(imageBytes);
        final ui.FrameInfo frameInfo = await codec.getNextFrame();
        embeddedImage = frameInfo.image;
      }
      ...
      final pdfData = await _exportService.exportToPdf(painter);

      final params = SaveFileDialogParams(
        data: pdfData,
        fileName: 'qrx_pro_code.pdf',
      );
  ...
  Future<void> _saveQrCode() async {
    setState(() => _isSavingPng = true); // Use specific state
    try {
      ...
      if (mounted) setState(() => _isSavingPng = false); // Use specific state
    }
  }

  Future<void> _shareQrCode() async {
    setState(() => _isSharing = true); // Use specific state
    try {
      ...
      if (mounted) setState(() => _isSharing = false); // Use specific state
    }
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          // --- FIX #2: Update buttons to use their own state variables ---
          Wrap(
            spacing: 12.0,
            runSpacing: 12.0,
            ...
            children: [
              FilledButton.tonalIcon(
                onPressed: isProcessing ? null : _saveQrCode,
                icon: _isSavingPng
                    ? const SizedBox(
                        width: 18,
                        height: 18,
              ...
              FilledButton.tonalIcon(
                onPressed: isProcessing ? null : _shareQrCode,
                icon: _isSharing
                    ? const SizedBox(
                        width: 18,
                        height: 18,
              ...
              FilledButton.tonalIcon(
                onPressed: isProcessing ? null : _exportQrAsPdf,
